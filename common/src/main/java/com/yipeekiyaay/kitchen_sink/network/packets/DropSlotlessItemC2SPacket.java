...
public record DropSlotlessItemC2SPacket(int slotlessItemIndex, boolean isHoldingCtrl) implements CustomPayload {
    ...
    public static void handle(DropSlotlessItemC2SPacket payload, NetworkManager.PacketContext context) {
        context.queue(() -> {
            ...
            handleCommon(index, isHoldingCtrl, player);
        });
    }

    public static void handleCommon(int index, boolean isHoldingCtrl, PlayerEntity player) {
